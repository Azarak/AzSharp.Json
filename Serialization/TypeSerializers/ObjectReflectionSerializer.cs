    ...
    public object? Deserialize(JsonNode node, object? obj, Type type, int version)
    {
        ...
        foreach (FieldInfo field in fieldInfo)
        {
            ...
            if (!dict.ContainsKey(attribute.Tag))
            {
                ...
                {
                    // Guess put some error in here since we didn't get a required field?
                }
                continue;
            }
            object? value = JsonSerializer.Deserialize(null, dict[attribute.Tag], field.FieldType, attribute.TypeSerializer, version);
        ...
        foreach (PropertyInfo property in propertyInfo)
        {
            ...
            object? value = JsonSerializer.Deserialize(null, dict[attribute.Tag], property.PropertyType, attribute.TypeSerializer, version);
